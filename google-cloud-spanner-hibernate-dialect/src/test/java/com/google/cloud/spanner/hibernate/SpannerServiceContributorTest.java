...
public class SpannerServiceContributorTest {
    ...
    public void testUserAgentContribution() throws SQLException {
        // create mock Driver so we can intercept the call to
        // Driver.connect(url, props)
        Driver mockDriver = mock(Driver.class);
        when(mockDriver.connect(any(), any())).thenReturn(
        ...
        // verify that our mock Driver.connect(url, props) was called and
        // capture the properties
        ArgumentCaptor<Properties> propertiesArgumentCaptor = ArgumentCaptor
                .forClass(Properties.class);
        verify(mockDriver, atLeastOnce()).connect(any(String.class),
                propertiesArgumentCaptor.capture());

        // verify the the userAgent was passed in with properties
        assertThat(propertiesArgumentCaptor.getValue().getProperty("userAgent"))
                .isEqualTo("sp-hib");
    }
